...
####   Adapted from:  https://github.com/wiktorolszowy/fMRI_temporal_autocorrelation/blob/master/analysis_for_one_subject_FSL.R
###############################################################################################
...
path_output        = paste0(path_scratch, "/analysis_output_", study, "/FSL")
...
system(paste0("cp ", path_manage, "/FSL_FEAT_designs/design_preproc.fsf ", path_output, "/preproc_feats_designs/design_", subject, ".fsf"))
setwd(paste0(path_output, "/preproc_feats_designs"))
...
new_output = paste0(path_output, "/preproc_feats/", subject, "_preproc")
...
#-slice timing
def_slices    = "slice_timing_replace"
def_file      = "slice_timing_file_replace"
if (study=="CamCAN_sensorimotor") {
   new_slices = 2
   new_file   = ""
} else if (study=="NKI_release_3_checkerboard_1400" || study=="NKI_release_3_checkerboard_645"){
   new_slices = 0
   new_file   = ""
} else if (study=="BMMR_checkerboard") {
   new_slices = 5
   new_file   = ""
} else if (study=="CRIC_checkerboard") {
   new_slices = 4
   #-https://www.jiscmail.ac.uk/cgi-bin/webadmin?A2=ind1703&L=FSL&D=0&1=FSL&9=A&J=on&d=No+Match%3BMatch%3BMatches&z=4&P=381801
   slice_timing = seq(0.5, -0.5, length.out=33)[1:32]
   slice_order  = c(seq(2,32,by=2), seq(1,31,by=2))
   slice_timing[slice_order] = slice_timing
   write(slice_timing, paste0(path_data, "/", subject, "_", task, "_bold_slice_timing.txt"), ncolumns=1)
   new_file     = paste0(path_data, "/", subject, "_", task, "_bold_slice_timing.txt")
}
system(paste0("sed -i 's:'", def_slices, "':'", new_slices, "':g' ", "design_", subject, ".fsf"))
system(paste0("sed -i 's:'", def_file,   "':'", new_file,   "':g' ", "design_", subject, ".fsf"))
...
for (exper_design in exper_designs) {
   ...
   for (HRF_model in HRF_models) {
      ...
      system(paste0("rm -rf ", subject))
      system(paste0("rm -rf ", subject, ".feat"))

      #-copy the FEAT dir from the 'preproc' step
      system(paste0("cp -a ", path_output, "/preproc_feats/", subject, "_preproc.feat/ ", path_output_exper_design_HRF, "/"))
      ...
      if (exper_design=="event1" && study=="CamCAN_sensorimotor") {
         new_exper_design = paste0(path_manage, "/experimental_designs/CamCAN_sensorimotor/FSL_", strrep("0", 4-nchar(paste(subject_id))), subject_id, ".txt")
      } else {
      ...
      new_feat = paste0(path_output, "/exper_design_", exper_design, "/HRF_", HRF_model, "/", subject, ".feat")
      system(paste0("sed -i 's:", def_feat, ":", new_feat, ":g' design_", subject, ".fsf"))

      #-to change FIR parameters, change the design.fsf file
