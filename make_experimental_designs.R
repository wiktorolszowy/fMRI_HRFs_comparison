...
write(randomOnsets,    file="SPM_event2.txt",    sep="\n")
...
for (subject_id in 1:621) {
   setwd(paste0(path_scratch, "/scans_CamCAN_sensorimotor"))
   event_times = read.table(file=paste0("sub-CAS", strrep("0", 4-nchar(paste(subject_id))), subject_id, "_sensorimotor_events.tsv"), sep="\t", header=T)
   onsets      = event_times$onset
   setwd(paste0(path_manage, "/experimental_designs/CamCAN_sensorimotor"))
   #-AFNI
   for (i in 1:length(onsets)) {
      if (i==1) {
         AFNI_output = paste0(onsets[i], ":0.1")
      } else {
         AFNI_output = paste0(AFNI_output, " ", onsets[i], ":0.1")
      }
   }
   write(AFNI_output, file=paste0("AFNI_", strrep("0", 4-nchar(paste(subject_id))), subject_id, ".txt"), ncolumns=3)
   #-FSL
   FSL_output     = array(NA, dim=c(3,length(onsets)))
   FSL_output[1,] = round(onsets, digits=2)
   FSL_output[2,] = rep(0.1,length(onsets))
   FSL_output[3,] = rep(1,  length(onsets))
   write(FSL_output, file=paste0("FSL_", strrep("0", 4-nchar(paste(subject_id))), subject_id, ".txt"), ncolumns=3)
   #-SPM
   write(event_times$onset, file=paste0("SPM_", strrep("0", 4-nchar(paste(subject_id))), subject_id, ".txt"), sep="\n")
}

#-add CamCAN-event1 design for other datasets
setwd(paste0(path_scratch, "/scans_CamCAN_sensorimotor"))
event_times = read.table(file=paste0("sub-CAS0001_sensorimotor_events.tsv"), sep="\t", header=T)
onsets      = event_times$onset
setwd(paste0(path_manage, "/experimental_designs"))
for (study_id in 2:length(studies)) {
   study             = studies[study_id]
   npts              = studies_parameters$npts[study_id]
   TR                = studies_parameters$TR[study_id]
   onsets_truncated  = onsets[which(onsets<npts*TR)]
   #-AFNI
   for (i in 1:length(onsets_truncated)) {
      if (i==1) {
         AFNI_output = paste0(onsets_truncated[i], ":0.1")
      } else {
         AFNI_output = paste0(AFNI_output, " ", onsets_truncated[i], ":0.1")
      }
   }
   write(AFNI_output, file=paste0("AFNI_", study, "_event1.txt"), ncolumns=3)
   #-FSL
   FSL_output        = array(NA, dim=c(3,length(onsets_truncated)))
   FSL_output[1,]    = round(onsets_truncated, digits=2)
   FSL_output[2,]    = rep(0.1,length(onsets_truncated))
   FSL_output[3,]    = rep(1,  length(onsets_truncated))
   write(FSL_output, file=paste0("FSL_", study, "_event1.txt"), ncolumns=3)
}
#-SPM
write(onsets, file=paste0("SPM_event1.txt"), sep="\n")
